...
namespace AutomationOverhaul.Content.Machines.Pistons
{
    public class HellstonePiston : ModTile
    {
        ...
        public override void SetStaticDefaults() {
            ...
            RegisterItemDrop(ModContent.ItemType<HellstonePistonItem>());
        ...
        public override void PlaceInWorld(int i, int j, Item item) {
            ...
            int id = ModContent.GetInstance<HellstonePistonTE>().Place(i, j);

            if (id == -1) {
                Main.NewText($"[Error] Failed to spawn Piston Brain at {i},{j}!", Microsoft.Xna.Framework.Color.Red);
            }
            else {
                if (TileEntity.ByID.TryGetValue(id, out TileEntity te) && te is HellstonePistonTE piston) {
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem) {
            if (!fail) {
                ModContent.GetInstance<HellstonePistonTE>().Kill(i, j);
            }
        }
